...
Num_Étudiant : exactement 8 chiffres (CHAR(8) en base)
"""
...
def new_student():
    if request.method == "POST":
        ...
        # CONTRÔLE STRICT : 8 chiffres
        if len(num) != 8 or not num.isdigit():
            flash("Numéro étudiant : exactement 8 chiffres.", "danger")
            ...
            try:
                ...
                # 2) Création du compte à 0.00 €
                cursor.execute("""
                    INSERT INTO Compte (Num_Etudiant, Solde_Actuel)
                    VALUES (%s, 0.00)
                """, (num,))

                # 3) Crédit d'accueil 1.00 € (offre de bienvenue)
                cursor.callproc(
                    "CrediterCompte",
                    [num, 1.00, "Offre de bienvenue (1€)"]
                )

                cnx.commit()
                flash("Étudiant créé avec une offre de bienvenue de 1,00 €.", "success")
    ...
    <form method="post" style="max-width:500px;">
      <div class="mb-3">
        ...
        <div class="form-text">Exemple : 01234567</div>
      </div>
      <div class="mb-3">
        <label>Nom</label>
      ...
      <div class="mb-2">
        <small class="text-muted">
          Un crédit de bienvenue de 1,00 € sera automatiquement ajouté au compte.
        </small>
      </div>
...
def add_bonus():
    if request.method == "POST":
        ...
        commentaire_base = request.form.get("commentaire", "").strip()
    ...
    <form method="post" style="max-width:500px;">
      <div class="mb-3">
        <label>Numéro étudiant (8 chiffres)</label>
        <input class="form-control" name="num" required maxlength="8">
      </div>
      <div class="mb-3">
        <label>Montant (€)</label>
