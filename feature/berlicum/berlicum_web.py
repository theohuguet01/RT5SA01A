...
def verify_pin(pin_str):
    ...
    try:
        ...
        if sw1 == 0x90 and sw2 == 0x00:
            ...
            return False, f"PIN incorrect. Essais restants: {sw2}"
...
<head>
    ...
    <style>
        * {
        ...
        .card-animation {
            margin: 40px auto;
            animation: cardInsert 2s ease 0.6s both;
        }
        .card-animation.waiting {
            animation: cardWaiting 2s ease infinite;
        }
...
<body>
    <div class="background-animation">
        ...
        <!-- Petites formes qui traversent l'écran -->
        <div class="floating-shape circle" style="width: 30px; height: 30px; top: 30%; animation-name: slide; animation-duration: 25s; animation-iteration-count: infinite; animation-delay: 0s;"></div>
    ...
    <div class="container">
        ...
        <div id="menuContainer" class="menu-container">
            ...
            <div id="infoDisplay" style="display: block;">
                ...
                <button class="continue-button" onclick="showMainMenu()">
    ...
    <script>
            ...
            while (!cardDetected) {
                try {
                    ...
                    if (data.success) {
                        ...
                        // Carte détectée - changer l'animation
                        ...
                        welcomeTitle.style.animation = 'fadeInDown 0.8s ease';
                        ...
                        // Attendre 3.5 secondes pour laisser l'animation se terminer
                        setTimeout(() => {
                            welcomeScreen.classList.remove('active');
                            menuContainer.classList.add('active');
                            
                            // Afficher les informations dans la carte
                        ...
                        // Pas de carte détectée, réessayer après 2 secondes
                        await new Promise(resolve => setTimeout(resolve, 2000));
                    }
                } catch (error) {
                    // Erreur de connexion, réessayer après 2 secondes
                    await new Promise(resolve => setTimeout(resolve, 2000));
                }
            }
        }
        ...
        async function afficherInfos() {
            // Fonction conservée pour compatibilité mais non utilisée dans le nouveau flux
            showResult(`Numéro: ${studentData.num_etudiant}\nNom: ${studentData.nom}\nPrénom: ${studentData.prenom}`, 'success');
        }
        ...
        async function submitPin() {
            ...
            if (currentAction === 'transfert') {
                ...
                try {
                    ...
                    const data = await response.json();
                    if (data.success) {
                        showResult(`Solde disponible: ${data.solde} €`, 'info');
                    } else {
                        showResult(data.message, 'error');
                    }
                } catch (error) {
        ...
        // Handler pour la recharge finale
        window.addEventListener('load', function() {
            const originalSubmitPin = submitPin;
            submitPin = async function() {
                const pin = document.getElementById('pinInput').value;
                
                if (currentAction === 'recharge_confirm' && currentData.montant) {
                    closeModal();
                    document.getElementById('pinInput').value = '';
...
def api_recharge():
    ...
    if not ok_bdd:
        return jsonify({
            'success': True,
            'message': f"Carte créditée ({montant:.2f} €) mais erreur BDD"
        })
